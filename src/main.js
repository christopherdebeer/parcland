class CanvasController {
    ...
    buildContextMenu(elId) {
        ...
        this.contextMenu.appendChild(typesContainer);
        ...
        types.forEach(t => {
            ...
            this.clickCapture(btn, () => {
                el.type = t.type;
                this.updateElementNode(this.elementNodesMap[el.id], el, (el.id === this.selectedElementId));
                this.saveCanvas();
            });
        ...
        this.contextMenu.appendChild(blendSelect);
        const blends = [
        ...
        if (el.type === "img") {
            ...
            this.contextMenu.appendChild(regenBtn);
        }
        ...
        if (el.type === 'text' || el.type === 'markdown') {
            ...
            this.contextMenu.appendChild(colorInput);
        }
        ...
        this.clickCapture(staticBtn, () => {
            let node = this.elementNodesMap[el.id];
            this.updateElementNode(node, el, true);
            this.toggleStatic(el);
            if (el.static) {
                this.staticContainer.appendChild(node);
            } else {
                this.container.appendChild(node);
        ...
        this.contextMenu.appendChild(staticBtn);
        ...
        if (el.type === 'canvas-container' && el.childCanvasState) {
            ...
            this.contextMenu.appendChild(openCanvasBtn);
        }
        ...
        this.clickCapture(deleteBtn, () => {
            this.canvasState.elements = this.canvasState.elements.filter(e => e.id !== el.id);
            if (this.elementNodesMap[el.id]) {
                this.elementNodesMap[el.id].remove();
                delete this.elementNodesMap[el.id];
            }
            this.selectedElementId = null;
            this.hideContextMenu();
            this.saveCanvas();
        ...
        this.clickCapture(duplicateBtn, () => {
            ...
            newEl.x += 20;
            newEl.y += 20;
            this.canvasState.elements.push(newEl);
            this.selectElement(newEl.id);
            this.hideContextMenu();
            this.renderElements();
            this.saveCanvas();
        });
        this.contextMenu.appendChild(duplicateBtn);

        const idEl = document.createElement("span")
        idEl.classList.add("id-label");
        idEl.innerHTML = el.id;
        this.contextMenu.appendChild(idEl);
